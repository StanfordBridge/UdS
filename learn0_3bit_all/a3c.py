...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        #self.out==Tensor(actor/FullyConnected_4/Softmax:0", shape=(?, 3), dtype=float32)
        ...
        # This gradient梯度 will be provided by the critic network
        self.act_grad_weights = tf.compat.v1.placeholder(tf.float32, [None, 1])

        # Compute the objective (log action_vector and entropy)计算目标(log action_vector和熵)
        ...
        # Combine the gradients here在这里合并梯度
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, learning_rate):
        ...
        # Network target目标 V(s)
        self.td_target = tf.compat.v1.placeholder(tf.float32, [None, 1])

        # Temporal Difference, will also be weights for actor_gradients时间差异，也将是actor_gradients的权重
        self.td = tf.subtract(self.td_target, self.out)

        # Mean square error均方误差
        self.loss = tflearn.mean_square(self.td_target, self.out)

        # Compute critic gradient计算临界梯度
...
def compute_gradients(s_batch, a_batch, r_batch, terminal, actor, critic):
    ...
    if terminal:
        ...
        R_batch[-1, 0] = v_batch[-1, 0]  # bootstrap启动 from last state
